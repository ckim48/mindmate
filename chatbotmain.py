...
client = OpenAI(api_key='sk-proj-PS9LCW5kUHelsf7XkKJzT3BlbkFJltaxZ0vMjjldco8n7Uet')
...
def get_response(prompt, model="gpt-3.5-turbo"):
    ...
    response= client.chat.completions.create(
        model='gpt-3.5-turbo',
        messages=[
            {'role': 'user', 'content': prompt+"If it is greeting or bye message, then answer to one senetence.  Otherwise two sentences: one sentence for words of comfort and Give the solutions in one sentence"}
        ],
        temperature=0
    )
